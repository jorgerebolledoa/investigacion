...
def crear_visualizaciones(df):
    ...
    axes[0,0].set_title('Tiempo de Ejecución (Escala Log)')
    axes[0,0].legend()
    ...
    axes[1,0].plot(df['Nodos'], df['Eficiencia_Distribuido'], 'o-', label='Distribuido', color='blue', linewidth=2, markersize=8)
    axes[1,0].plot(df['Nodos'], df['Eficiencia_Secuencial'], 's-', label='Secuencial', color='red', linewidth=2, markersize=8)
    ...
    plt.tight_layout()
    plt.savefig('analisis_rendimiento_400.png', dpi=300, bbox_inches='tight')
    plt.show()

def mostrar_tabla_resumen(df):
    """Muestra una tabla resumen de los datos"""
    print("\n" + "="*120)
    print("TABLA RESUMEN: ANÁLISIS DE RENDIMIENTO - 400 PUNTOS")
    print("="*120)
...
if __name__ == "__main__":
    ...
    df.to_csv('analisis_400_puntos.csv', index=False)
    print("\nDatos guardados en 'analisis_400_puntos.csv'")
    print("Gráficos guardados en 'analisis_rendimiento_400.png'")
